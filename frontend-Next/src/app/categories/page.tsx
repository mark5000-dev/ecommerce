...
export default function CategoriesPage() {
  // Leverage TanStack Query hook for unified caching, lifecycle states, and refetching
        ...
        <section className="py-20 bg-background border-b border-border">
          <div className="container mx-auto px-4 lg:px-8">
            {isLoading ? (
              ...
              <div className="text-center py-20 font-serif tracking-wide text-destructive">
                Critical failure during categories synchronization. Please try again later.
              </div>
            ) : categories.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {categories.map((category: Category) => (
                  // Safe fallback key to check both potential identity property namings
                  <CategoryCard key={category.categoryId || category.categoryId} category={category} />
                ))}
              </div>
            )}
          </div>
